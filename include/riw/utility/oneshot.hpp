...
namespace riw
{
  ...
  public:
    ...
    ValueType get_out() const
    {
      ...
      const auto v = std::clamp(static_cast<ValueType>(std::chrono::duration_cast<std::chrono::milliseconds>(d).count()) / static_cast<float>(duration.count()), static_cast<ValueType>(0), static_cast<ValueType>(1));
